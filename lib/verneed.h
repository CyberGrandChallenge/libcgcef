...
#error libcgcef currently does not support VER_NEED_CURRENT != 1
...
#define translator(x,e)	xlt3(xltprefix(_cgcef_##x),e,translator_suffix)
...
xlt_verneed(unsigned char *dst, const unsigned char *src, size_t n, unsigned enc) {
	...
	/* never happens for CGCEF v1 and Verneed v1 */
...
translator(verneed,L11)(unsigned char *dst, const unsigned char *src, size_t n) {
    return xlt_verneed(dst, src, n, CGCEFDATA2LSB);
}

size_t
translator(verneed,M11)(unsigned char *dst, const unsigned char *src, size_t n) {
    return xlt_verneed(dst, src, n, CGCEFDATA2MSB);
}
