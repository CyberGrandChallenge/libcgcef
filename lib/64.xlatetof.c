/*
 * 64.xlatetof.c - implementation of the cgcef64_xlateto[fm](3) functions.
 ...
 */
copy_type(addr_64,_,CGCEf64_Addr,copy_addr_11)
copy_type(half_64,_,CGCEf64_Half,copy_half_11)
copy_type(off_64,_,CGCEf64_Off,copy_off_11)
copy_type(sword_64,_,CGCEf64_Sword,copy_sword_11)
copy_type(word_64,_,CGCEf64_Word,copy_word_11)
copy_type(sxword_64,_,CGCEf64_Sxword,copy_sxword_11)
copy_type(xword_64,_,CGCEf64_Xword,copy_xword_11)
copy_type(dyn_64,11,CGCEf64_Dyn,copy_dyn_11)
copy_type(ehdr_64,11,CGCEf64_Ehdr,copy_ehdr_11)
copy_type(phdr_64,11,CGCEf64_Phdr,copy_phdr_11)
copy_type(rela_64,11,CGCEf64_Rela,copy_rela_11)
copy_type(rel_64,11,CGCEf64_Rel,copy_rel_11)
copy_type(shdr_64,11,CGCEf64_Shdr,copy_shdr_11)
copy_type(sym_64,11,CGCEf64_Sym,copy_sym_11)

typedef size_t (*xlator)(unsigned char*, const unsigned char*, size_t);
typedef xlator xltab[CGCEF_T_NUM][2];
...
xlate64_11[/*encoding*/] = {
	...
	{ _cgcef_verdef_64L11_tom,  _cgcef_verdef_64L11_tof  },
	{ _cgcef_verneed_64L11_tom, _cgcef_verneed_64L11_tof },
#else /* __LIBCGCEF_SYMBOL_VERSIONS */
	{ 0,                0               },
	{ 0,                0               },
...
#if __LIBCGCEF_SYMBOL_VERSIONS
	{ _cgcef_verdef_64M11_tom,  _cgcef_verdef_64M11_tof  },
	{ _cgcef_verneed_64M11_tom, _cgcef_verneed_64M11_tof },
#else /* __LIBCGCEF_SYMBOL_VERSIONS */
	{ 0,                0               },
	{ 0,                0               },
#endif /* __LIBCGCEF_SYMBOL_VERSIONS */
 ...
 */
size_t
_cgcef64_xltsize(const CGCEf_Data *src, unsigned dv, unsigned encode, int tof) {
    CGCEf_Type type = src->d_type;
 ...
 */
static CGCEf_Data*
cgcef64_xlate(CGCEf_Data *dst, const CGCEf_Data *src, unsigned encode, int tof) {
    CGCEf_Type type;
    int dv;
    int sv;
    size_t dsize;
 ...
 */
CGCEf_Data*
cgcef64_xlatetom(CGCEf_Data *dst, const CGCEf_Data *src, unsigned encode) {
    return cgcef64_xlate(dst, src, encode, 0);
}

CGCEf_Data*
cgcef64_xlatetof(CGCEf_Data *dst, const CGCEf_Data *src, unsigned encode) {
    return cgcef64_xlate(dst, src, encode, 1);
}

CGCEf_Data*
gcgcef_xlatetom(CGCEf *cgcef, CGCEf_Data *dst, const CGCEf_Data *src, unsigned encode) {
    if (cgcef) {
	if (cgcef->e_kind != CGCEF_K_CGCEF) {
	    seterr(ERROR_NOTCGCEF);
	}
	else if (cgcef->e_class == CGCEFCLASS32) {
	    return cgcef32_xlatetom(dst, src, encode);
	}
	else if (cgcef->e_class == CGCEFCLASS64) {
	    return cgcef64_xlatetom(dst, src, encode);
	}
...
CGCEf_Data*
gcgcef_xlatetof(CGCEf *cgcef, CGCEf_Data *dst, const CGCEf_Data *src, unsigned encode) {
    if (cgcef) {
	if (cgcef->e_kind != CGCEF_K_CGCEF) {
	    seterr(ERROR_NOTCGCEF);
	}
	else if (cgcef->e_class == CGCEFCLASS32) {
	    return cgcef32_xlatetof(dst, src, encode);
	}
	else if (cgcef->e_class == CGCEFCLASS64) {
	    return cgcef64_xlatetof(dst, src, encode);
	}
	else if (valid_class(cgcef->e_class)) {
	    seterr(ERROR_UNIMPLEMENTED);
	}
