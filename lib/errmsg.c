/*
 * errmsg.c - implementation of the cgcef_errmsg(3) function.
...
#if HAVE_CATGETS
# include <nl_types.h>
static nl_catd _libcgcef_cat = (nl_catd)0;
#endif /* HAVE_CATGETS */

#if HAVE_DGETTEXT || HAVE_CATGETS
static const char domain[] = "libcgcef";
...
#if HAVE_CATGETS
    if (_libcgcef_cat == (nl_catd)0) {
	_libcgcef_cat = catopen(domain, 0);
    }
    if (_libcgcef_cat != (nl_catd)-1) {
	return catgets(_libcgcef_cat, 1, err + 1, _messages[err]);
    }
#endif /* HAVE_CATGETS */
